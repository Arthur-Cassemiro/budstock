    ...
    def produto_novo(self):
        ...
        if not check:
            if self.numero and self.nome and self.preço and self.quantidade:
                ...
                self.preço, self.quantidade, lista[0]))
                conn.commit()
    ...
    def alterar_produto(self, nro_prod):
        ...
        if not check or int(self.numero) == int(nro_prod):
            ...
            if not check or check[0] == nro_prod:
                ...
                if self.numero and self.nome and self.preço and self.quantidade:
                    c.execute("UPDATE "+self.estoque_nome+" SET numero=?, nome=?, preço=?, quantidade=?  WHERE numero=?",
...
#@app.route("/")
#def teste():
#    return render_template('Budstock.html')
...
def pag_estoque():
    ...
    if x == 1:
        ...
        for prods in estoque.mostrar_estoque():
            z = str(w)
            prod = Produto(request.form["numero"+z],request.form["nome"+z],request.form["quantidade"+z],request.form["preço"+z], estoque.estoque)
            print(prod.numero)
            print(prod.nome)
            print(prod.preço)
        ...
        prodnovo = Produto(request.form["numeron"],request.form["nomen"],request.form["preçon"],request.form["quantidaden"], estoque.estoque)
    ...
    for info in prod:
        ...
        if z == 3:
            ...
            y=[]
        z = (z+1)% 4
    return render_template('repasse2.html', li_li_tup = li_li_tup, estoque = "estoque_teste")
